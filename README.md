...
{
  "extraction": {
    "countSql": "SELECT COUNT(*) as total FROM ${SCHEMA}.patron t1 ${JOIN_TABLE} WHERE t1.last_name is not null ${JOIN_WHERE}",
    "pageSql": "SELECT t1.patron_id, t1.institution_id, t1.last_name, t1.first_name, t1.middle_name, nvl2(t1.expire_date, to_char(t1.expire_date,'YYYYMMDD'), to_char(t1.purge_date,'YYYYMMDD')) AS active_date, nvl2(t1.expire_date, to_char(t1.expire_date,'YYYY-MM-DD'), to_char(t1.purge_date,'YYYY-MM-DD')) AS expire_date, t1.sms_number, t1.current_charges FROM ${SCHEMA}.patron t1 ${JOIN_TABLE} WHERE t1.last_name is not null ${JOIN_WHERE} ORDER BY patron_id OFFSET ${OFFSET} ROWS FETCH NEXT ${LIMIT} ROWS ONLY",
    "usernameSql": "SELECT uin, tamu_netid FROM PATRON.person_identifiers WHERE uin = '${INSTITUTION_ID}'",
